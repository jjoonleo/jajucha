...
class Graphics:
    def __init__(self, classPlanning):
        ...
        self.txtTime.grid(row=4, column=1, sticky='w', pady=(120, 0))
        self.txtTime.grid_propagate(False)

        self.DriveModeFrame = tkinter.Frame(self.control_frame)
        self.InitializeModeFrame = tkinter.Frame(
            self.control_frame, bg="#000000")
        self.ImageModeFrame = tkinter.Frame(self.control_frame)

        self.runMode = tkinter.IntVar()
        self.runMode.set(2)
        self.runMode.trace("w", lambda *args: self.callback_change_mode())
        self.btnModeDrive = tkinter.Radiobutton(self.DriveModeFrame, text="주행 모드",
                                                value=0, variable=self.runMode)
        self.btnModeImage = tkinter.Radiobutton(self.ImageModeFrame, text="재생 모드",
                                                value=1, variable=self.runMode)
        self.btnModeInitialize = tkinter.Radiobutton(self.InitializeModeFrame, text="초기화 모드",
                                                     value=2, variable=self.runMode)
        self.btnModeDrive.grid(row=1, column=1, sticky='w')
        self.btnModeInitialize.grid(row=1, column=1, sticky='w')
        self.btnModeImage.grid(row=1, column=1, sticky='w')

        self.runSave = tkinter.IntVar()
        self.runSave.set(0)
        self.runSave.trace("w", lambda *args: self.callback_change_save())
        self.btnDriveSave = tkinter.Checkbutton(self.DriveModeFrame, text="주행 저장",
        ...
        self.lblDriveSave = tkinter.Label(
            self.DriveModeFrame, textvariable=self.varTxtSavePath)
        self.btnFindSavePath = tkinter.Button(
            self.DriveModeFrame, text="폴더 찾기", command=self.callback_save_dir)
        ...
        self.lblImageLoad = tkinter.Label(
            self.ImageModeFrame, textvariable=self.varTxtLoadPath)
        self.btnFindLoadPath = tkinter.Button(
            self.ImageModeFrame, text="폴더 찾기", command=self.callback_load_dir)
        self.lblImageLoad.grid(row=2, column=1, sticky='w')
        self.btnFindLoadPath.grid(row=2, column=2, sticky='w')
        self.loadLocation = None

        self.ImageControlFrame = tkinter.Frame(self.ImageModeFrame)
        ...
        self.ImageControlFrame.grid(row=3, column=1, sticky='w')
        ...
        self.btnStartStop.grid(row=5, column=1, sticky='nwse')

        self.DriveModeFrame.grid(row=1, column=1, sticky='w')
        self.InitializeModeFrame.grid(
            row=2, column=1, sticky='w', pady=(60, 0))
        self.ImageModeFrame.grid(row=3, column=1, sticky='w', pady=(60, 0))

        try:
            self.client = communication.Client('tcp://%s:%d' % config.address)
            if self.client.id is None:
                self.setCommandText('자주차가 사용중입니다.')
                if tkinter.messagebox.askyesno('연결 오류', '자주차가 사용중입니다.\n강제로 연결할까요?'):
                    if self.client.override():
                        self.btnStartStop['state'] = tkinter.NORMAL
                        self.setCommandText('자주차에 연결되었습니다.')
                    else:
    ...
    def callback_change_mode(self):
        ...
        if mode == 0:  # Drive Mode
            ...
            self.control = control.DriveControl(self, config.address, self.client)
            ...
            self.control.quit()
            self.control = control.InitializationControl(self, config.address, self.client)
