...
class BaseControl:
    ...
    def process(self, inV, inL, inR, t, _fImg,  _rImg,  _fLdr,  _rLdr):
        self.graphics.plan.pre_process(t, _fImg, _rImg, _fLdr, _rLdr)
        try:
            command = self.graphics.plan.process(
                inV, inL, inR, t, _fImg, _rImg, _fLdr, _rLdr)
...
class DriveControl(BaseControl):
    def __init__(self, graphics, address, client):
    ...
    def _run(self):
        try:
            ...
            self.graphics.setCommandText('초기화를 먼저 진행해 주세요')
        ...
        try:
            ...
            while not self._halt:
                ...
                self._rImg = self._fImg
                self.graphics.setFrontImage1(self._fImg)
                self.graphics.setRearImage1(self._rImg)

                command = self.process(
                    self.inV, self.inL, self.inR, self.t, self._fImg, self._rImg, self._fLdr, self._rLdr)
...
class InitializationControl(BaseControl):
    def __init__(self, graphics, address, client):
        super().__init__(graphics)
        self.address = address
        self.receiver = communication.VideoStreamSubscriber(
    ...
    def f(self, width, distacne, r):
        return math.acos((r-width)/r)*r-distacne

    def df(self, width, r):
        tmp = math.acos((r-width)/r)
        tmp2 = (1/r - (r-width)/r**2)*r
        tmp3 = math.sqrt(1-(r-width)**2/r**2)
    ...
    def _run(self):
        try:
            ...
            while not self._halt:
                ...
                command = self.graphics.plan.initSteer(t, self._fImg)
                s, v = command
                ...
                if v == 0:
                    ...
                    r = 14
                    for i in range(0, 14):
                        f = self.f(width, distance, r)
                        #print(f)
                        df = self.df(width, r)
                        #print(df)
                        r = -(f/df) + r
                        #print(r)
                    print("cameraCircleRadius:",r)
                    cameraPointDistance = 16
                    R = math.sqrt((r**2) - (cameraPointDistance**2))
                    print("carCircleRadius:",R)
                    ackermanAngle = (180/math.pi)*math.atan(21/R)
                    print("ackermanAngle=", ackermanAngle)
                    self._halt = True
    ...
    def start(self):
        ...
        self.graphics.btnModeDrive['state'] = tkinter.DISABLED
        self.graphics.btnModeImage['state'] = tkinter.DISABLED
        self.graphics.varTxtStartStop.set('준비')
        self.graphics.plan.init(self._fImg)
        self._startTime = time.time()
        self._live = True
