...
class Camera:
    def __init__(self, mtx, h, trans, trans_inv, flip=False):
        ...
        #                            [0., 1, 0],
        #                            [0., 0, 1]], dtype=np.float32unWarpPts) @ \
        #                  np.array([[f_u, c_u, 0],
        #                            [0., c_v, h * f_v],
...
class BasePlanning:
    ...
    def initialize(self, time, frontImage):
        frontLines, frontObject = self.processFront(frontImage)

    def process(self, initialV, initialL, initialR, time, frontImage, rearImage, frontLidar, rearLidar):
        frontLines, frontObject = self.processFront(frontImage)
    ...
    def initSteer(self, time, frontImage):
        V, L, R = self.gridFront(frontImage, cols=7, rows=3)
        velocity = 20
